...
def create_connectedk8s(cmd, client, resource_group_name, cluster_name, correlation_id=None, https_proxy="", http_proxy="", no_proxy="", proxy_cert="", location=None,
    ...
    utils.helm_install_release(cmd.cli_ctx.cloud.endpoints.resource_manager, chart_path, subscription_id, kubernetes_distro, kubernetes_infra, resource_group_name, cluster_name,
                               ...
                               enable_private_link, arm_metadata, kube_aad_endpoint, onboarding_timeout, container_log_path)
...
def enable_features(cmd, client, resource_group_name, cluster_name, features, kube_config=None, kube_context=None,
    ...
    if enable_cl:
        if cl_oid is None:
            raise RequiredArgumentMissingError("Custom locations object id was not passed. Please pass '--custom-locations-oid' parameter to enable custom locations.")
        subscription_id = os.getenv('AZURE_SUBSCRIPTION_ID') if custom_token_passed is True else get_subscription_id(cmd.cli_ctx)
...
def custom_locations_oid_valid(cmd, cl_oid):
    ...
    try:
        ...
        result = list(sp_graph_client.list(filter=("objectId eq '{}'".format(cl_oid))))
        logger.debug(f"Retrieved SP app named '{result[0].display_name}' for object id {cl_oid}")
        return True
    except Exception as e:
