...
def create_connectedk8s(cmd, client, resource_group_name, cluster_name, correlation_id=None, https_proxy="", http_proxy="", no_proxy="", proxy_cert="", location=None,
    ...
    cloud_name = send_cloud_telemetry(cmd)
    ...
    # Validates and gets values from Helm values file for a custom environemnt.
    dp_endpoint_custom = None
    release_train_custom = None
    if values_file_provided:
        cloud_name, dp_endpoint_custom, release_train_custom = validate_env_file_custom(values_file, values_file_provided)
    ...
    if release_namespace:
        ...
        if connected_cluster_exists(client, configmap_rg_name, configmap_cluster_name):
            if (configmap_rg_name.lower() == resource_group_name.lower() and
                ...
                if enable_private_link is True:
                    disable_cluster_connect(cmd, client, resource_group_name, cluster_name, kube_config, kube_context, values_file, values_file_provided, dp_endpoint_custom, release_train_custom, release_namespace, helm_client_location)
                return cc_response
            else:
    ...
    registry_path = os.getenv('HELMREGISTRY') if os.getenv('HELMREGISTRY') else utils.get_helm_registry(cmd, config_dp_endpoint, dp_endpoint_custom, release_train_custom)
    ...
    utils.helm_install_release(chart_path, subscription_id, kubernetes_distro, kubernetes_infra, resource_group_name, cluster_name,
                               ...
                               kube_context, no_wait, values_file_provided, values_file, cloud_name, disable_auto_upgrade, enable_custom_locations,
...
def validate_env_file_custom(values_file, values_file_provided):
    if not values_file_provided:
        ...
        raise ValidationError("Helm environment file is required when using custom environment for onboarding the cluster.", recommendation="Please set the environment variable 'HELMVALUESPATH' to point to the file.")
    ...
    with open(values_file, 'r') as f:
        ...
        try:
            assert env_dict.get('global').get('azureEnvironment')
            assert env_dict.get('systemDefaultValues').get('azureArcAgents').get('config_dp_endpoint_override')
            assert env_dict.get('systemDefaultValues').get('azureArcAgents').get('releaseTrain')
        except Exception as e:
            telemetry.set_exception(exception=e, fault_type=consts.Helm_Environment_File_Fault_Type,
                                    summary='Problem loading the helm environment variables')
            raise FileOperationError("The required helm environment variables for custom onboarding are either not present in the file or incorrectly set.", recommendation="Please check the values 'global.azureEnvironment' and 'systemDefaultValues.azureArcAgents.config_dp_endpoint_override' in the file.")

    # Return the dp endpoint and release train
    cloud_name = env_dict.get('global').get('azureEnvironment')
    ...
    return cloud_name, dp_endpoint, release_train
...
def update_connected_cluster(cmd, client, resource_group_name, cluster_name, https_proxy="", http_proxy="", no_proxy="", proxy_cert="",
    ...
    release_train_custom = None
    if values_file_provided:
        dp_endpoint_custom, release_train_custom = validate_env_file_custom(values_file, values_file_provided)[1:3]
...
def disable_features(cmd, client, resource_group_name, cluster_name, features, kube_config=None, kube_context=None,
    ...
    get_chart_and_disable_features(cmd, connected_cluster, dp_endpoint_custom, release_train_custom, kube_config, kube_context,
...
def get_chart_and_disable_features(cmd, connected_cluster, dp_endpoint_custom, release_train_custom, kube_config, kube_context,
...
def disable_cluster_connect(cmd, client, resource_group_name, cluster_name, kube_config, kube_context, values_file, values_file_provided, dp_endpoint_custom, release_train_custom, release_namespace, helm_client_location):
    # Fetch Connected Cluster for agent version
    connected_cluster = get_connectedk8s(cmd, client, resource_group_name, cluster_name)
